...
使用 `--sampling-interval` 启用抽样模式，按时间间隔聚合数据，显著降低长时间范围的数据量。

**字段聚合语义：**

对于 `node_stats` 和 `index_stats`，抽样模式下会对字段进行分类处理：

| 字段类型 | 计算方式 | 示例字段 |
|---------|---------|---------|
| **Rate 型** | `delta(value) / bucket_size` | `index_total`, `query_total`, `search_time_in_millis` |
| **Latency 型** | `delta(time) / delta(count)` | `query_latency`, `index_latency` |
| **Max 型** | 取桶内最大值 | `heap_used_in_bytes`, `cpu.percent` |
| **未分类** | 保留 latest 快照值 | 其他字段 |

**输出形态：**
- 抽样后输出覆盖原字段为聚合值
- `--slim` 仅裁剪元数据字段，不影响聚合值
- 未分类字段按 typed fallback（数值取 avg，字符串取 majority）

**抽样示例：**
```bash
# 15分钟间隔抽样导出
python metrics-exporter/metrics_exporter.py -c http://localhost:9000 -u admin -p password \
  --time-range 168 --sampling-interval 15m

# 1小时间隔抽样，精简输出
python metrics-exporter/metrics_exporter.py -c http://localhost:9000 -u admin -p password \
  --time-range 720 --sampling-interval 1h --slim
```

**并行与分页一致性：**
- 抽样模式支持 `--parallel-degree` 并行导出
- `rate_state` 按 `(group_key, field)` 维护状态，确保多页与并行下导数稳定
- 不同节点/索引的状态相互隔离，无串扰
