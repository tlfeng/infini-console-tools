...
```bash
# 推荐用于大时间范围导出的高性能参数组合
# 注意：zsh 下密码包含 ! 时，建议先 set +H 或使用单引号包裹密码
set +H && python metrics-exporter/metrics_exporter.py \
  -c http://localhost:9000 \
  -u admin \
  -p 'Qwer123!' \
  --time-range 1 \
  --metric-types cluster_health,node_stats \
  --sampling-interval 15m \
  --parallel 2 \
  --parallel-degree 2 \
  -o exports/perf-smoke
```

说明：
- `--sampling-interval 15m` 适合先做快速抽样导出，显著降低长时间范围的数据量。
- `--parallel 2` 用于指标间并行，`--parallel-degree 2` 用于单指标内部并行。
- 当前参数组合已在 `localhost:9000` 测试环境验证通过，适合作为首选起点；若 ES 负载稳定，可逐步提高到 `--parallel 3` 或 `--parallel-degree 4`。
